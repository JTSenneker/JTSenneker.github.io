...
mtlLoader.load('./src/models/sphere.mtl', function (materials) {
    ...
    objLoader.load('./src/models/sphere.obj', function (object) {
        object.traverse(function (node) {
            if (node.isMesh) node.material = new THREE.MeshBasicMaterial({ map: map });
        });
        object.scale.x = 15;
        object.scale.z = 15;
        object.scale.y = 15;
...
function rotateCam() {
    cam.rotation.y += (targetXRot - cam.rotation.y) * .1;
...
function updateHotspots() {
    ...
    for (var i = 0; i < hotspots.length; i++) {
        if (hotspots[i].mouseOver) {
            ...
            this.tl.to(hotspots[i].scale, 1, { x: 2, y: 2, z: 2, ease: Expo.easeOut });
        } else {
            this.tl = new TimelineMax();
            this.tl.to(hotspots[i].scale, 1, { x: 1, y: 1, z: 1, ease: Expo.easeOut });
        }
    }
}
function makeVideoPlane() {
    ...
    var vidMat = new THREE.MeshBasicMaterial({ map: vidTexture, side: THREE.DoubleSide });
